...
// //получение картинок на кнопках управления дорожкой
// const btnPp = document.querySelectorAll('.track-menu__btn2 img');
// const btnTl = document.querySelectorAll('.track-menu__btn1 img');
// const btnTr = document.querySelectorAll('.track-menu__btn3 img');

// //получаем меню дорожки
// const PBtns = document.querySelectorAll('.track-menu>div');

// //получаем title и artist
// const tTitle = document.querySelector('.track__title');
// const tArtist = document.querySelector('.track__artist');

// //получаем доступ к полосе
// const trackPaint = document.querySelector('.track-line__paint-line');
// //элемент управления
// const controlBtn = document.querySelector('.controlBtn');

// //получаем черную линию
// const trackLine = document.querySelector('.track-line__line');

const player = document.querySelectorAll('.header-player');
// audioElement.title = 'Веселые гвоздики';
// audioElement.artist = 'Владимир Фесенко';

let NewPlayer = new AudioPlayer(player);

// NewPlayer.playPause(trackTitle,trackArtist,playerPlayeBtns,btnPlayPause,trackPaint,controlBtn,trackLine);



// audioElement.addEventListener('timeupdate', function (event) {
//     //вычисляем заполнение progress bar
//     trackPaint.style.width = `${audioElement.currentTime / audioElement.duration * 100}%`;
//     //позиционируем элемент управления
//     controlBtn.style.left = `${audioElement.currentTime / audioElement.duration * 100 - 4.4}%`;
// });
...
// //получаем координаты и разницу в стартовых положениях кнопки управления и строки
// const coordBtnStart = controlBtn.getBoundingClientRect().left;
// const diffLineBtn = trackLine.getBoundingClientRect().left - controlBtn.getBoundingClientRect().left;

// //функция устанавливает позицию элемента
// function setPosition(evt) {
//     //вычисляем позиционирование в %
//     let positionLeft = (evt.clientX - coordBtnStart - diffLineBtn - controlBtn.clientWidth / 2);
//     positionLeft /= trackLine.clientWidth / 100;

//     //по позиционированию вычисляем currentTime для трека (при timeupdate выше уже элемент будет спозиционирован)
//     audioElement.currentTime = (positionLeft + 4.4) / 100 * audioElement.duration;
// };

// trackLine.addEventListener(`dragover`, function (event) {
//     //разрешаю перемещать элементы в эту область
//     event.preventDefault(); //отменяем действие по умолчанию
